...
class LikeSongsListView extends StatefulWidget {
  const LikeSongsListView({super.key});

  @override
  State<LikeSongsListView> createState() => _LikeSongsListViewState();
}

class _LikeSongsListViewState extends State<LikeSongsListView> {
  ...
  void initState() {
    ...
    super.initState();
    // Get.find<ProfileController>().viewMoreSongList.clear();
    ...
    return Scaffold(
      body: Obx(() {
        return CustomBackground(
          child: Get.find<ProfileController>().isLikeSongsLoading.value
              ? const Center(
                  child: CircularProgressIndicator(
                    color: Colors.white,
                  ),
                )
              : Get.find<ProfileController>().likeSongs.isEmpty
                  ? SafeArea(
                      child: Center(
                        child: SmallText(
                          text: "No Data Found!",
                                            ...
                                            Padding(
                                              padding:
                                                  const EdgeInsets.fromLTRB(
                                                      60, 8, 60, 0),
                                              child: Image.network(
                                                "${Api.baseUrl}/${Get.find<ProfileController>().likeSongs[0].thumbnail320}",
                                                // width: MediaQuery.of(context)
                                                //     .size
                                                //     .width,
                                                fit: BoxFit.fill,
      ...
      return GestureDetector(
        onTap: () {
          if (Get.find<GetXPlayerController>().playlistNotifier.isEmpty) {
            ...
            //     .playlistNotifier
            //     .isEmpty) {
            var list = Get.find<ProfileController>().likeSongs;
            List<MediaItem> playlist = [];
